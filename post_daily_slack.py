...
# Explicit mapping for WHS custom week numbers (within the CURRENT YEAR)
...
TOPIC_EMOJIS = {
    ...
    # Optional: add emojis for your new topics (safe fallback exists if omitted)
    "STAIR": {
        "header": ":ladder:",
        "title": ":warning:",
        "footer": "Safe-to-go :safetogo:",
...
def week1_start_sunday(year: int) -> date:
    ...
    jan1 = date(year, 1, 1)
    # weekday(): Mon=0 ... Sun=6
    # days back to previous Sunday (or 0 if it's already Sunday)
...
# ---------------------------------------------------------------
#  SELECT WEEKLY TOPIC (SUNDAY–SATURDAY, WEEK 1 CONTAINS JAN 1)
# ---------------------------------------------------------------
def pick_weekly_topic(topics_json: dict, today: date | None = None) -> dict:
    """
    Choose this week's topic.

    1) Compute WHS week number for the current YEAR:
Weeks run Sunday–Saturday
Week 1 starts on the Sunday on or before Jan 1

    2) If WHS week number is explicitly mapped in WHS_WEEK_TOPIC_CODES,
       select that topic code.

    3) Otherwise, fall back to rotation by modulo across all topics in JSON.
    """
    if today is None:
        today = date.today()
    ...
    # 1) Compute WHS week number for the year
    custom_week_number = whs_week_number(today)
...
def pick_message_for_today() -> str:
    ...
    return build_slack_text(topic, message)
